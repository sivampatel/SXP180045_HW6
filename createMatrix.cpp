...
void createMatrix()
{
  ...
  */
  ...
  if (myMatrix ==NULL)
    {
      std::cout << "Error creating matrix. Exiting." << std::endl;
      _exit(1);
    }
  ...
  // open binary file
  binaryFile.open("/scratch/perkins/cs3377.bin", std::ios::in | std::ios::binary);
  ...
  st << "Magic: 0x" << std::hex << std::uppercase << head->magicNumber;  
  setCDKMatrixCell(myMatrix, 1, 1, st.str().c_str());
  st.str(std::string());

  std::stringstream ss;  // stringstream for everything else besides Magic Number
